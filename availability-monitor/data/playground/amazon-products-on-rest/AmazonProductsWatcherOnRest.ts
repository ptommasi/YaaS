...
export class AmazonProductsWatcherOnRest extends LinkWatcher {
  ...
  async prepareChunk(chunk: Product[], iteration: number) {
    ...
    const browserLocker = await new BrowserLocker().boot(String.fromCharCode('A'.charCodeAt(0) + iteration));
    ...
    for (let product of chunk) {
      ...
      const allTLDs = TLDs.map(async (tld) => {
        ...
        const bp = new AmazonProductPageOnRest({ tld, groupName, product, headersManager });
