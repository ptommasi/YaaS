...
export class AmazonProductPageOnRest extends AbstractWatcher {
  ...
  private readonly headersManager: HeadersManager;
  ...
  constructor(params: AmazonPageOnRestParams) {
    ...
    this.headersManager = params.headersManager;
    this.logger = new Logger(this.id);
  }

  async prepare() {
    // loggerWithId.info(this.id, `Headers for chrome loaded for page ${this.url} (domain is ${this.tld}, gpu is ${this.product.title}).`);
  }
  ...
  async start() {
    ...
    while (true && !this.stop) {
      ...
      const requestHeaders = this.headersManager.headers;
      ...
      if (merchantResult === "BotDetected" || merchantResult === "CaptchaActive") {
        ...
        if (consecutiveReloads > 2) {
          ...
          await this.headersManager.restart();
          if(this.headersManager.headers.cookies === requestHeaders.cookies) {
            this.logger.error("We have a bigger problem, after restart cookies didn't change.");
          }
          continue;
        } else if (consecutiveReloads > 1) {
