  ...
  async prepareChunk(chunk: Product[], iteration: number) {
    ...
    for (let product of chunk) {
      ...
      const allTLDs = TLDs.map(async (tld) => {
        ...
        bp.Heartbeat.on(hb => {
          this.countsMap[tld]++;
          if (this.countsMap[tld] % 500 === 0){
            this.logger.info(`${this.countsMap[tld]} total checks done on www.amazon.${tld} (running time: ${inMinutes(Date.now() - this.startTime)}).`);
          }
          this.overallCount++;
          if (this.overallCount % 1000 === 0){
            this.logger.info(`${this.overallCount} total checks done among all amazon domains (running time: ${inMinutes(Date.now() - this.startTime)}).`);
          }
